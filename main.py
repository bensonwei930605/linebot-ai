...
# 老闆的 LINE User ID (會從 Render 環境變數自動讀取)
BOSS_LINE_USER_ID = os.environ.get('BOSS_LINE_USER_ID', '')


@app.route("/", methods=['GET'])
def home():
    return "Wei IT & Bike Assistant Server is Running!", 200
...
def handle_message(event):
    ...
    核心任務與規則：
    1. 【車款與購車諮詢】：當客戶詢問買車推薦、預算、規格或 CP 值時，**絕對不要盲目在網路上幫忙推薦車款**。你必須稱讚客人的方向，並告訴對方：「這部分需要由店長（老闆）親自幫您評估現貨與最適合的搭配，我已經通知老闆了，老闆稍後會親自回覆您！」
    2. 【維修與保養諮詢】：針對異音排除、226 賽前大保養、變速箱調校等技術問題，可以給予專業、簡短的初步解答。
    3. 【行程與預約管理】：當客戶有明確預約維修或看車的時間需求時，請幫忙整理資訊，並在回覆結尾附上 Google 日曆加入連結：
